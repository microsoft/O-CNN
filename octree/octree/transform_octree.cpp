...
void ScanOctree::set_scale(float scale) {
  scale_ = scale;
}
...
void octree_dropout(vector<char>& octree_output, const string& octree_input,
  ...
  vector<vector<unsigned int> > drop(depth + 1);
  ...
  if (location_feature == -1) {
    std::copy_n(parser_in.feature_cpu(0), num * channel_feature, parser_out.mutable_feature_cpu(0));
  }
  int channel_label = parser_in.info().channel(OctreeInfo::kLabel);
  int location_label = parser_in.info().locations(OctreeInfo::kLabel);
  if (location_label == -1) {
    std::copy_n(parser_in.label_cpu(0), num * channel_label, parser_out.mutable_label_cpu(0));
  }
  int channel_split = parser_in.info().channel(OctreeInfo::kSplit);
  int location_split = parser_in.info().locations(OctreeInfo::kSplit);
  if (location_split == -1) {
    std::copy_n(parser_in.split_cpu(0), num * channel_split, parser_out.mutable_split_cpu(0));
  }
  ...
  for (int d = depth_dropout + 1; d <= depth; ++d) {
    ...
    const unsigned int* key_src = parser_in.key_cpu(d);
    int* child_des = parser_out.mutable_children_cpu(d);
    unsigned int* key_des = parser_out.mutable_key_cpu(d);
