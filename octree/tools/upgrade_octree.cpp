...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < all_files.size(); i++) {
    ...
    int nnum_depth = octree_info.nnum(depth);
    memcpy(oct_parser.mutable_key(0), key, total_nnum * sizeof(int));
    memcpy(oct_parser.mutable_child(0), children, total_nnum * sizeof(int));
    int data_size = FLAGS_adaptive ? total_nnum : nnum_depth;
    memcpy(oct_parser.mutable_feature(0), data, data_size * data_channel * sizeof(float));
    if (FLAGS_node_label) {
      const int* ptr = data + data_size * data_channel;
      float* des = oct_parser.mutable_label(0);
    ...
    if (FLAGS_split_label) {
      const int* ptr = data + data_size * data_channel;
      float* des = oct_parser.mutable_split(0);
