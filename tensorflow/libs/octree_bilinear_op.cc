...
class OctreeBilinearOP : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES_OK(context, context->allocate_temp(DT_UINT32, rst_shape, &buf_xyz));
    auto rst_xyz = buf_xyz.flat<uint32>().data();
    ...
    auto rst_key = buf_key.flat<uint32>().data();    
 ...
 protected:
  ...
  void key2xyz_gpu_op(OpKernelContext* context, Tensor* xyz_tensor,
      const uint32* key, const int num, const int depth) {
    OP_REQUIRES_OK(context,
        context->allocate_temp(DT_UINT32, TensorShape({ num }), xyz_tensor));
    auto ptr = xyz_tensor->flat<uint32>().data();
