...
def get_variables_with_name(name=None, without=None, train_only=True, verbose=False):
  ...
  t_vars = tf.compat.v1.trainable_variables() if train_only else tf.compat.v1.all_variables()
...
def dense(inputs, nout, use_bias=False):
  ...
  fc = tf.compat.v1.layers.dense(inputs, nout, use_bias=use_bias,
                       kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
  return fc


def batch_norm(inputs, training, axis=1):
  return tf.compat.v1.layers.batch_normalization(inputs, axis=axis, training=training)
...
def conv2d(inputs, nout, kernel_size, stride, padding='SAME', data_format='channels_first'):
  return tf.compat.v1.layers.conv2d(inputs, nout, kernel_size=kernel_size, strides=stride,
                          padding=padding, data_format=data_format, use_bias=False,
                          kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))


def octree_conv1x1(inputs, nout, use_bias=False):
  outputs = tf.compat.v1.layers.conv2d(inputs, nout, kernel_size=1, strides=1,
...
def upsample(data, channel, training):
  deconv = tf.compat.v1.layers.conv2d_transpose(data, channel, kernel_size=[8, 1],
                                      strides=[8, 1], data_format='channels_first', use_bias=False,
                                      kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
  bn = tf.compat.v1.layers.batch_normalization(deconv, axis=1, training=training)
  return tf.nn.relu(bn)


def downsample(data, channel, training):
  deconv = tf.compat.v1.layers.conv2d(data, channel, kernel_size=[8, 1],
                            strides=[8, 1], data_format='channels_first', use_bias=False,
                            kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
  bn = tf.compat.v1.layers.batch_normalization(deconv, axis=1, training=training)
  return tf.nn.relu(bn)
...
def avg_pool2d(inputs, data_format='NCHW'):
  return tf.nn.avg_pool2d(input=inputs, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                          padding='SAME', data_format=data_format)


def global_pool(inputs, data_format='channels_first'):
  axis = [2, 3] if data_format == 'channels_first' else [1, 2]
  return tf.reduce_mean(input_tensor=inputs, axis=axis)


# !!! Deprecated
def octree_upsample(data, octree, depth, channel, training):
  with tf.compat.v1.variable_scope('octree_upsample'):
...
def octree_downsample(data, octree, depth, channel, training):
  with tf.compat.v1.variable_scope('octree_downsample'):
...
def octree_conv_bn(data, octree, depth, channel, training, kernel_size=[3],
  ...
  return tf.compat.v1.layers.batch_normalization(conv, axis=1, training=training)
...
def octree_deconv_bn_relu(data, octree, depth, channel, training, kernel_size=[3],
                          stride=1, fast_mode=False):
  with tf.compat.v1.variable_scope('deconv_bn_relu'):
...
def predict_label(data, num_output, num_hidden, training):
  ...
  label = tf.argmax(input=logit, axis=1, output_type=tf.int32)  # predict (1, ?, 1)
...
def softmax_loss(logit, label_gt, num_class, label_smoothing=0.0,infoGain=False):
  with tf.compat.v1.name_scope('softmax_loss'):
    ...
    if infoGain :
      ...
      loss = tf.compat.v1.losses.sparse_softmax_cross_entropy(label_gt, logit, weights)
    else:  
      onehot = tf.one_hot(label_gt, depth=num_class)
      loss = tf.compat.v1.losses.softmax_cross_entropy( onehot, logit, label_smoothing=label_smoothing)
  return loss


def l2_regularizer(name, weight_decay):
  with tf.compat.v1.name_scope('l2_regularizer'):
...
def label_accuracy(label, label_gt):
  label_gt = tf.cast(label_gt, tf.int32)
  accuracy = tf.reduce_mean(input_tensor=tf.cast(tf.equal(label, label_gt), dtype=tf.float32))
  return accuracy


def softmax_accuracy(logit, label):
  with tf.compat.v1.name_scope('softmax_accuracy'):
    predict = tf.argmax(input=logit, axis=1, output_type=tf.int32)
    accu = label_accuracy(predict, tf.cast(label, tf.int32))
  return accu


def regress_loss(signal, signal_gt):
  return tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.square(signal-signal_gt), axis=1))
...
def solver_multiple_gpus(total_loss, learning_rate_handle, gpu_num):
  ...
  for i in range(gpu_num):
    with tf.device('/gpu:%d' % i):
      with tf.compat.v1.name_scope('device_b%d' % i):
  ...
  with tf.device('/cpu:0'):
    with tf.compat.v1.name_scope('sync_and_apply_grad'):
      with tf.control_dependencies(update_ops):
...
def summary_train(names, tensors):
  with tf.compat.v1.name_scope('summary_train'):
    summaries = []
    for it in zip(names, tensors):
      summaries.append(tf.compat.v1.summary.scalar(it[0], it[1]))
    summ = tf.compat.v1.summary.merge(summaries)
  return summ


def summary_test(names):
  with tf.compat.v1.name_scope('summary_test'):
    summaries = []
    summ_placeholder = []
    for name in names:
      summ_placeholder.append(tf.compat.v1.placeholder(tf.float32))
      summaries.append(tf.compat.v1.summary.scalar(name, summ_placeholder[-1]))
    summ = tf.compat.v1.summary.merge(summaries)
...
def loss_functions_seg(logit, label_gt, num_class, weight_decay, var_name, mask=-1,infoGain=False):
  with tf.compat.v1.name_scope('loss_seg'):
    label_mask = tf.greater(label_gt,-1)  # filter label -1
    masked_logit = tf.boolean_mask(tensor=logit, mask=label_mask)
    masked_label = tf.boolean_mask(tensor=label_gt, mask=label_mask)
...
def get_seg_label(octree, depth):
  with tf.compat.v1.name_scope('seg_label'):
...
def tf_IoU_per_shape(pred, label, class_num, mask=-1):
  with tf.compat.v1.name_scope('IoU'):
    ...
    for k in range(class_num):
      ...
      intsc = tf.reduce_sum(input_tensor=tf.cast(pk & lk, dtype=tf.float32))
      union = tf.reduce_sum(input_tensor=tf.cast(pk | lk, dtype=tf.float32))
      valid = tf.cast(tf.reduce_any(input_tensor=lk), dtype=tf.float32)
...
class Optimizer:
  ...
  def __call__(self, total_loss, learning_rate):
    with tf.compat.v1.name_scope('solver'):
      update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
      with tf.control_dependencies(update_ops):
        global_step = tf.Variable(0, trainable=False, name='global_step')
