...
void OctreeDepaddingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void OctreeDepaddingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  int top_h = octree_batch_.info().node_num_nempty(curr_depth_);
  if (top_h == bottom_h) {
    LOG_EVERY_N(INFO, 1000) << "The layer " << this->layer_param_.name()
        << " might be redundant.";
  } else {
...
template <typename Dtype>
void OctreeDepaddingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  if (top_h != bottom_h) {
    octree::pad_backward_cpu<Dtype>(top_data, top_h, channel,
...
template <typename Dtype>
void OctreeDepaddingLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (top_h != bottom_h) {
    octree::pad_forward_cpu<Dtype>(bottom_diff, bottom_h, channel,
...
#ifdef CPU_ONLY
STUB_GPU(OctreeDepaddingLayer);
#endif

INSTANTIATE_CLASS(OctreeDepaddingLayer);
REGISTER_LAYER_CLASS(OctreeDepadding);
