...
TYPED_TEST(EltwiseLayerTest, TestMin) {
	typedef typename TypeParam::Dtype Dtype;
	LayerParameter layer_param;
	EltwiseParameter* eltwise_param = layer_param.mutable_eltwise_param();
	eltwise_param->set_operation(EltwiseParameter_EltwiseOp_MIN);
	shared_ptr<EltwiseLayer<Dtype> > layer(
		new EltwiseLayer<Dtype>(layer_param));
	layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
	layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
	...
	for (int i = 0; i < count; ++i) {
		EXPECT_EQ(data[i],
			std::min(in_data_a[i], std::min(in_data_b[i], in_data_c[i])));
	}
}

 TYPED_TEST(EltwiseLayerTest, TestMinGradient) {
 	typedef typename TypeParam::Dtype Dtype;
 	LayerParameter layer_param;
 	EltwiseParameter* eltwise_param = layer_param.mutable_eltwise_param();
 	eltwise_param->set_operation(EltwiseParameter_EltwiseOp_MIN);
